...
pred_times = [pd.to_datetime("09:30:00").time(), pd.to_datetime("15:30:00").time(), pd.to_datetime("21:30:00").time(), pd.to_datetime("03:30:00").time()]   # prediction times at which new latest GFS model will be available
...
for idx in range(1 + history, aws_standardized.shape[0] - forecast_horizon + 1):        # 1 + history because we are going to remove aws_array[0] and considered history*15 minutes of observed history; -forecast_horizon + 1 because we will sum the values till total forecast_horizon value is obtained, so last few indices will give error
#     if(idx not in indices_to_be_adjusted):
    aws_features.append(aws_standardized[idx - history:idx])
    y.append(aws_standardized[idx:idx + forecast_horizon].sum(axis=0))
...
# gfs_features_adjusted_path = "/Users/kpmac/Documents/IITB/DS593/data/preprocessed/gfs_features_adjusted_array.npy"
# np.save(gfs_features_adjusted_path, gfs_features)
# 
# gfs_features_adjusted_path = "/Users/kpmac/Documents/IITB/DS593/data/preprocessed/gfs_features_adjusted_array.npy"
# gfs_features = np.load(gfs_features_adjusted_path)
...
#     dtest_reg = xgb.DMatrix(X_test, y_test[:, i])
#     params = {
...
#     xgb_test_preds = model.predict(dtest_reg)
#     print(regions[i], "Training score:", r2_score(y_train[:, i], xgb_train_preds), "Validation score:", r2_score(y_val[:, i], xgb_val_preds), "Test score:", r2_score(y_test[:, i], xgb_test_preds))
#     print()
...
for i in range(y.shape[1]):
    ...
    y_test2 = np.asarray(y_test[:, i], dtype=np.float32)
    ...
    test_dataset = TensorDataset(X_test, y_test2)
    print("dataset")
    ...
    print("scheduler")
    checkpoint_path = "/Users/kpmac/Documents/IITB/DS593/model/checkpoints/nn0/nn0_" + str(i) + ".pth"
    ...
    with torch.no_grad():
        ...
        print("Training score: %.4f" % r2_score(y_train2, y_pred_train), "Validation score: %.4f " % r2_score(y_val2, y_pred_val), "Test score: %.4f" % r2_score(y_test2, y_pred_test), regions[i])
